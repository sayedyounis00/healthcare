...
class AddEditMedicineSheet extends StatefulWidget {
  ...
  final void Function(models.MedicineModel medicine) onSave;
...
class _AddEditMedicineSheetState extends State<AddEditMedicineSheet> {
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          spacing: 8,
          runSpacing: 8,
          children: models.MedicineTags.predefined.map((tag) {
  ...
  void _saveMedicine() {
    if (_formKey.currentState!.validate()) {
      ...
      final medicine = models.MedicineModel(
        id: widget.medicine?.id ?? models.MedicineModel.generateId(),
        patientId: widget.medicine?.patientId ?? 'current_patient',
        drugName: _nameController.text.trim(),
        description: _descriptionController.text.trim(),
        timesToTake: _selectedTimes
            .map((t) => models.TimeOfDay(hour: t.hour, minute: t.minute))
            .toList(),
        tags: _selectedTags,
